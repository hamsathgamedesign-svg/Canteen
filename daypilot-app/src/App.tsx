...
const API_BASE_URL = import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:4000').replace(/\/$/, '')
...
function App() {
  ...
  const submitAdminLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({ error: 'The admin API did not return JSON.' })) as { error?: string; token?: string; username?: string }
      if (!response.ok || !result.token) throw new Error(result.error || 'Invalid admin credentials')
  ...
  const handleLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({ error: 'The account API did not return JSON. Please redeploy the latest DayPilot build.' })) as { error?: string; token?: string }
